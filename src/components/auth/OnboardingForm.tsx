...
export function OnboardingForm({ email }: OnboardingFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // 2. Set password via Supabase auth
      const { error: pwError } = await supabase.auth.updateUser({ password })
      if (pwError) throw new Error(pwError.message)

      // 3. Save profile via API route
      ...
      toast.success('Account set up! Welcome to PixelFlow.')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        <Label>Email</Label>
        <Input value={email} disabled className="bg-gray-50 text-slate-400 cursor-not-allowed" />
      </div>
