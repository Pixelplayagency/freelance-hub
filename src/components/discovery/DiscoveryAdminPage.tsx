...
import type { DiscoveryToken, DiscoverySubmission } from '@/lib/types/app.types'
...
function SubmissionDetail({ s }: { s: DiscoverySubmission }) {
  ...
  return (
    <div className="rounded-xl border-2 overflow-hidden" style={{ borderColor: '#e5e0d8' }}>
      ...
      >
        <div>
          <p className="text-sm font-bold" style={{ color: '#1a1714' }}>
            {s.first_name} {s.last_name}
          </p>
          <p className="text-xs mt-0.5" style={{ color: '#8c8278' }}>
            {s.email} · {s.brand_name}
...
function TokenCard({ token, onDelete }: { token: DiscoveryToken; onDelete: (id: string) => void }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: isUsed ? 'rgba(16,185,129,0.3)' : isExpired ? '#e5e0d8' : '#e5e0d8', backgroundColor: 'white' }}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {token.label && (
              <span className="text-sm font-semibold truncate" style={{ color: '#1a1714' }}>{token.label}</span>
            )}
          </div>
          <p className="text-xs truncate" style={{ color: '#8c8278' }}>{url}</p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={copy}
            className="p-2 rounded-lg border transition-colors hover:bg-gray-50"
            style={{ borderColor: '#e5e0d8', color: copied ? '#059669' : '#8c8278' }}
            title="Copy link"
          >
          ...
          <button
            ...
            style={{ borderColor: '#e5e0d8', color: '#f24a49' }}
            title="Delete link"
          >
...
export function DiscoveryAdminPage({ tokens: initialTokens, adminId }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5 text-muted-foreground">
            Generate shareable links to send to potential clients
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Links', value: tokens.length },
          { label: 'Submitted', value: submitted.length },
          { label: 'Awaiting', value: active.length },
        ].map(s => (
      ...
      {tokens.length === 0 ? (
        <div className="bg-card border border-border rounded-2xl flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-xs">
            Create a link and send it to a potential client. They'll fill out a 2-page form with their brand details and needs.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {tokens.map(t => (
            <TokenCard key={t.id} token={t} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
