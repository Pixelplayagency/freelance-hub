  ...
  return (
    <div>
      ...
      {pending.length > 0 && (
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {pending.map(profile => (
              <div key={profile.id} className="bg-white rounded-lg border border-amber-200 p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-900 truncate">{profile.full_name ?? 'Unnamed'}</p>
      ...
      {active.length === 0 && pending.length === 0 ? (
        ...
        <div>
          {pending.length > 0 && (
            <h2 className="text-sm font-semibold text-slate-500 mb-3">Active ({active.length})</h2>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {active.map(profile => {
              const activeTasks = countMap[profile.id] ?? 0
              return (
                <div key={profile.id} className="bg-white rounded-lg border border-[#EBEBEB] p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="shrink-0 text-center">
                      <div className="text-sm font-bold text-slate-900">{activeTasks}</div>
                      <div className="text-[10px] text-slate-400 uppercase tracking-wide">tasks</div>
                    </div>
                  </div>
