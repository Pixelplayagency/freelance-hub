      ...
      {projectEntries.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {projectEntries.map(({ project, tasks: projectTasks, counts, total, completed }) => {
            ...
            return (
              <div key={project.id} className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 flex flex-col">

                {/* Cover banner — same h-28 as admin card */}
                ...
                {/* Body — same -mt-7 overlap as admin card */}
                <div className="px-4 pb-0 -mt-7 relative flex-1 flex flex-col">
                  ...
                  <div className="flex h-1.5 rounded-full overflow-hidden bg-border gap-px mb-1.5">
                    {STATUS_ORDER.map(s => {
                      ...
                      return (
                        <div key={s} className={cn('h-full', BAR_COLORS[s])} style={{ width: `${barPct}%` }} />
                      )
                    })}
                  </div>
                        ...
                        {pendingTasks.slice(0, 3).map(task => {
                          ...
                          return (
                            <Link
                              ...
                              className="flex items-center gap-2 py-1.5 px-2 -mx-2 rounded-lg hover:bg-muted transition-colors group"
                            >
                ...
                {/* Footer — same style as admin card */}
                <div className="px-4 py-3 border-t border-border flex items-center justify-between mt-3">
